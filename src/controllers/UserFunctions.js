// Require specific models so we can create functionality that involves them
...
// Transform encrypted data back into a plaintext string
function decryptString(data) {
  decipher = crypto.createDecipheriv(encAlgorithm, encPrivateKey, encIV);
  return decipher.update(data, 'hex', 'utf8') + decipher.final('utf8');
}

// Decrypt the string and transform it into a JavaScript object
...
async function getAllUsers() {
  // Return an array of raw MongoDB database documents
  return await User.find({})
}

async function getSpecificUser(userID) {
  // Return the raw MongoDB database document
...
module.exports = { encryptString, decryptString, decryptObject, hashString, validateHashedData, generateJWT, generateUserJWT, verifyUserJWT, getAllUsers, getSpecificUser, createUser, updateUser, deleteUser }